...
def generate_order_card(product_title: str, shop_name: str, price: str, product_image_url: str):
    ...
    try:
        ...
        # 1. Product Image
        try:
            # For testing, we might fail download, so let's mock if needed or use dummy
            # resp = requests.get(product_image_url, timeout=10)
            # resp.raise_for_status()
            # product_img = Image.open(BytesIO(resp.content)).convert("RGBA")
            product_img = Image.new('RGBA', (155, 149), color=(200, 200, 200, 255)) # Mock image
        ...
        filename = f"test_truncation_{uuid.uuid4().hex}.png"
