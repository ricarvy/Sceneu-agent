...
def build_agent(ctx=None):
    ...
    if system_prompt:
        system_prompt += (
            ...
            "4. **seed_mode 处理**：\n"
            "   - 你会收到一个 `seed_mode` 配置（通常在系统消息或用户消息中）。\n"
            "   - 如果 `seed_mode='true'`：你**必须**提取店铺名称（shop_name）和价格（price），并调用 `generate_marketing_image` 工具时传入 `seed_mode='true'`。这将触发订单截图生成。\n"
            "   - 如果 `seed_mode='false'`：无需强制提取店铺名和价格，调用工具时传入 `seed_mode='false'`。\n"
            "5. 对于多件商品，如果它们属于不同的店铺或有不同的价格，请将店铺名和价格也用英文逗号分隔合并，并分别传入 `shop_name` 和 `price` 参数（顺序需与商品一致）。\n"
            "6. 智能体生图时，将严格基于你传入的商品标题进行描述，而不会自行“看图说话”。"
            "\n\n# 文案与输出规范\n"
            "无论 `seed_mode` 是 `true` 还是 `false`，也无论模式是 `copy` 还是 `inspire`，都**必须严格**遵循以下输出规范：\n"
            "1. **严格字数限制**：只生成**一条**文案，总字数严格控制在 **100字以内**（不含标签）。不要长篇大论。\n"
            "2. **风格要求**：必须是小红书/朋友圈的“种草”或“日常分享”风，亲切自然，像朋友推荐。\n"
            "3. **格式规范**：\n"
            "   - 直接输出文案内容，不要有多余的引导语（如“这是为您生成的文案”）。\n"
            "   - 多用 Emoji (✨💖🔥) 增强氛围感，但不要堆砌。\n"
            "   - **绝对禁止**在文案中输出任何形式的链接（如 http://... 或 [链接](...)）。图片请通过 Markdown 图片格式展示。\n"
            "   - **绝对不要**在文案中输出文件路径或调试信息。\n"
            "4. **图片展示**：工具生成的图片 URL，请使用 Markdown 图片格式 `![](url)` 直接展示出来，不要直接把 URL 作为文本列出来。\n"
            "5. **示例**：\n"
            "   '今天的穿搭太绝了！✨ 这件[商品名]真的显瘦又高级，面料质感超棒，随手一拍就是大片 📸 姐妹们快冲！💖 #OOTD #日常穿搭'"
        )
